...
//		int MaxDate=theDate.get(Calendar.DAY_OF_MONTH);
    ...
    //日期加一年返回日期
    public static String addYearData(Date date) {
        Calendar cal = Calendar.getInstance();
        cal.setTime(date);
        cal.add(Calendar.YEAR, 1);
        Date addYear =  cal.getTime();
        return format(addYear,FORMAT_ALL);
    }
   ...
   /* public static void main(String[] args) {
        Date  date = parseString2Date("2015-10-10");
        String addYear =  addYearData(date);
        System.out.println("加一年后："+addYear);
        System.out.println("当前日期是否在这个范围："+belongCalendar(new Date(),parseString2Date("2017-12-21"),parseString2Date(addYearData(parseString2Date("2017-12-21")))));
    }*/
}
